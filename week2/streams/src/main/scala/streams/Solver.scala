...
trait Solver extends GameDef {
  ...
  def done(b: Block): Boolean =
    b.isStanding && b.b1 == goal && b.b2 == goal
  ...
  def from(
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val next = (for {
        list <- initial
        nex <-
          newNeighborsOnly(neighborsWithHistory(list._1, list._2), explored)
      } yield nex)
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = {
    val initial = LazyList((startBlock, List[Move]()))
    from(initial, Set[Block]())
  }
