...
class CodecsSuite
  ...
  /**
    ...
    */
  def encodeAndThenDecodeProp[A](
      a: A
  )(implicit encA: Encoder[A], decA: Decoder[A]): Prop = {
    val maybeDecoded = decA.decode(encA.encode(a))
    maybeDecoded.contains(
      a
    ) :| s"Encoded value '$a' was not successfully decoded. Got '$maybeDecoded'."
  }

  @Test def `it is possible to encode and decode the 'Unit' value (0pts)`()
      : Unit = {
  ...
  @Test def `the 'Int' decoder should reject invalid 'Int' values (2pts)`()
      : Unit = {
  ...
  @Test def `a 'String' value should be encoded as a JSON string (1pt)`()
      : Unit = {
    assert(implicitly[Encoder[String]].encode("foo") == Json.Str("foo"))
  }

  @Test def `it is possible to encode and decode 'String' values (1pt)`()
      : Unit = {
    checkProperty(Prop.forAll((s: String) => encodeAndThenDecodeProp(s)))
  }

  @Test def `a 'Boolean' value should be encoded as a JSON boolean (1pt)`()
      : Unit = {
  ...
  @Test def `it is possible to encode and decode 'Boolean' values (1pt)`()
      : Unit = {
    checkProperty(Prop.forAll((b: Boolean) => encodeAndThenDecodeProp(b)))
  }

  @Test def `a 'List[A]' value should be encoded as a JSON array (0pts)`()
      : Unit = {
    val xs = 1 :: 2 :: Nil
  ...
  @Test def `a 'Person' value should be encoded as a JSON object (1pt)`()
      : Unit = {
  ...
  @Test def `a 'Contacts' value should be encoded as a JSON object (1pt)`()
      : Unit = {
  ...
  @Test def `it is possible to encode and decode contacts (4pts)`(): Unit = {
    ...
    checkProperty(
      Prop.forAll(peopleGenerator)(people =>
        encodeAndThenDecodeProp(Contacts(people))
      )
    )
  }
