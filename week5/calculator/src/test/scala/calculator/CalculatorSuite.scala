...
class CalculatorSuite {
  ...
  @Test def `tweetRemainingCharsCount's result signal should follow the input signal`
      : Unit = {
  ...
  @Test def `Cyclic dependencies should result in NaN`: Unit = {
    ...
    val output = computeValues(input)
    val checkNaNs =
      output("a")().isNaN && output("b")().isNaN && output("c")().isNaN
    assert(
      checkNaNs,
  ...
  @Test def `Expressions corresponding to every variable should be correctly evaluated`
      : Unit = {
  ...
  @Test def `When an expression changes, other variables that depend on it should be recomputed`
      : Unit = {
    val aexpr = Var[Expr](Plus(Ref("b"), Literal(1)))
    val bexpr = Var[Expr](Times(Ref("c"), Ref("d")))
    val cexpr = Var[Expr](Plus(Literal(5), Ref("d")))
    ...
    val checkRes =
      (output("a")() != oldvals("a")) && (output("b")() != oldvals("b")) &&
  ...
  @Test def `If b previously depended on a, but no longer does, then it should not be recomputed anymore when a changes`
      : Unit = {
  ...
  @Test def `When an expression changes, *only* the variables that depend on it should be recomputed`
      : Unit = {
    val aexpr = Var[Expr](Plus(Ref("b"), Literal(1)))
